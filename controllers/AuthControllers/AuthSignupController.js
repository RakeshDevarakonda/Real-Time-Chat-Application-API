...
export const AuthSignupController = async (req, res, next) => {
  ...
  try {
    ...
    if (username.trim().length <= 3) {
      return res.status(400).json({ message: "Username must be at least 4 characters and non-empty spaces" });
    }
    ...
    if (cachedUser) {
      ...
      existingUser = await usercollections.findOne({ email });


      if (existingUser) {
        await redisClient.set(redisUserKey, JSON.stringify(existingUser), "EX", 3600);
        await redisClient.set(`user:id:${existingUser._id}`, JSON.stringify(existingUser), "EX", 3600);
        return res.status(409).json({ message: "User already exists" });
      }
    }
    ...
    const newUser = new usercollections({ username, email, password: hashedPassword });
    await newUser.save();
    // Cache the new user by email and ID
    await redisClient.set(redisUserKey, JSON.stringify(newUser), "EX", 3600);
    await redisClient.set(`user:id:${newUser._id}`, JSON.stringify(newUser), "EX", 3600);
